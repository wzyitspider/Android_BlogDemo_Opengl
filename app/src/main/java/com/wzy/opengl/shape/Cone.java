...
//    };
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        //绘制圆锥侧面
        shapePos = createPositions(2.0f);
        //绘制圆锥底面
        circlePos = createPositions(0.0f);
        ...
        // 初始化顶点字节缓冲区，用于存放形状的坐标，每个浮点数占用4个字节
        ByteBuffer cb = ByteBuffer.allocateDirect(circlePos.length * 4);
        //设置使用设备硬件的原生字节序
        cb.order(ByteOrder.nativeOrder());
        //从ByteBuffer中创建一个浮点缓冲区
        vertexBuffer2 = cb.asFloatBuffer();
        // 把坐标都添加到FloatBuffer中
        vertexBuffer2.put(circlePos);
        //设置buffer从第一个坐标开始读
